...
int main() {
    ...
    CHECK_CUDA(cudaMemcpy(
        d_a, h_a.data(), kNumElements * sizeof(float), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(
        d_b, h_b.data(), kNumElements * sizeof(float), cudaMemcpyHostToDevice));

    solve(d_a, d_b, d_c, kNumElements);
    CHECK_CUDA(cudaDeviceSynchronize());

    CHECK_CUDA(cudaMemcpy(
        h_c.data(), d_c, kNumElements * sizeof(float), cudaMemcpyDeviceToHost));
